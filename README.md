...
```bash
gsettings set org.freedesktop.ibus.general.hotkey \
  enable-unconditional "['Katakana']"
```

**入力切替キー設定**
- Mozc のキーマップ設定は「[2. 英数・かなキーの IME 切り替え設定](#2-英数かなキーの-ime-切り替え設定)」を参照。
...
Apple キーボードではトップ行のキーがデフォルトでメディアキー（輝度・音量等）として動作します。F1-F12 をデフォルトにするには、Toshy の `toshy-fnmode` ツールで `fnmode` を変更します:
